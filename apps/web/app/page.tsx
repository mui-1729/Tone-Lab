...
function AudioInput({ id, label, file, onChange }: {
  ...
  return (
    <section className="audio-card">
      ...
      <input id={id} className="sr-only" type="file" accept={ACCEPT}
...
function QualityStatus({ quality }: { quality: QualityInfo }) {
  const groups = [
    ["参考音", quality.reference.warnings],
    ["自分の音", quality.current.warnings],
    ["比較条件", quality.comparison_warnings],
  ] as const;
  ...
  return (
    <section className={`quality-card${warnings.length ? " quality-card-warning" : ""}`}>
      <div>
        <p className="eyebrow">INPUT CHECK / 入力状態</p>
        <p className="quality-message">{warnings.length ? "解析前に確認したい項目があります。" : "入力状態に大きな問題はありません。"}</p>
      </div>
...
function AdjustmentPlanSection({ steps }: { steps: AdjustmentStep[] }) {
  return (
    <section className="adjustment-section" aria-labelledby="adjustment-title">
          ...
          {steps.map((step, index) => (
            <article className="adjustment-card" key={step.key}>
              ...
              <div className="adjustment-heading">
                <div><p>{step.label} / 差 {Math.abs(step.difference).toFixed(0)}</p><h3>{step.title}</h3></div>
      ...
      ) : <div className="adjustment-empty">5つの質感軸では大きな調整は不要です。音量を揃えてA/Bし、耳で最終確認してください。</div>}
    </section>
  );
}
...
function ComparisonChart({ title, description, reference, current, min, max, leftLabel, rightLabel }: {
  title: string; description: string; reference: number[]; current: number[];
  ...
  return (
    <article className="visual-card">
      <div className="visual-heading">
        ...
        <div className="chart-legend" aria-label="グラフの凡例"><span className="legend-reference">参考音</span><span className="legend-current">自分の音</span></div>
      </div>
...
export default function Home() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try { setResult(await compareAudio(reference, current)); }
    catch (caught) { setError(caught instanceof Error ? caught.message : "解析に失敗しました。"); }
  ...
  return (
    <main>
      ...
      <form className="workspace" onSubmit={handleSubmit}>
        <div className="upload-grid"><AudioInput id="reference" label="REFERENCE / 参考音" file={reference} onChange={setReference} /><AudioInput id="current" label="CURRENT / 自分の音" file={current} onChange={setCurrent} /></div>
        <div className="conditions"><span>同じフレーズ推奨</span><span>開始位置を自動補正</span><span>最大30秒</span><span>WAV / MP3 / FLAC / OGG</span></div>
        <button className="analyze-button" type="submit" disabled={loading}>{loading ? "解析中…" : "2つの音を比較する"}</button>
      ...
      {result ? (
        <section className="results">
          ...
          <details className="raw-data"><summary>測定値を見る</summary><pre>{JSON.stringify({ alignment: result.alignment, quality: result.quality, reference: result.reference, current: result.current }, null, 2)}</pre></details>
