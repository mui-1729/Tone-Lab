...
function AudioInput({ id, label, file, onChange }: {
  id: string;
  label: string;
  file: File | null;
  ...
  return (
    <section className="audio-card">
      <div><p className="eyebrow">{label}</p><p className="file-name">{file?.name ?? "音源を選択してください"}</p></div>
      <label className="file-button" htmlFor={id}>ファイルを選ぶ</label>
      <input id={id} className="sr-only" type="file" accept={ACCEPT} onChange={(event) => onChange(event.target.files?.[0] ?? null)} />
...
function AlignmentStatus({ alignment }: { alignment: AlignmentInfo }) {
  ...
  if (absoluteOffset >= 0.02) {
    adjustment = offset > 0 ? `自分の音を約${absoluteOffset.toFixed(2)}秒前へ補正しました。` : `参考音を約${absoluteOffset.toFixed(2)}秒前へ補正しました。`;
  }
  return (
    ...
    <section className="adjustment-section" aria-labelledby="adjustment-title">
      ...
      {steps.length ? <div className="adjustment-grid">{steps.map((step, index) => (
        <article className="adjustment-card" key={step.key}>
          ...
          <ol>{step.actions.map((action) => <li key={action}>{action}</li>)}</ol><p className="adjustment-verify">{step.verify}</p>
        </article>
...
function ComparisonChart({ title, description, reference, current, min, max, leftLabel, rightLabel }: {
  title: string; description: string; reference: number[]; current: number[]; min: number; max: number; leftLabel: string; rightLabel: string;
}) {
  return (
    <article className="visual-card">
      ...
      <svg className="comparison-chart" viewBox={`0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`} role="img" aria-label={`${title}の参考音と自分の音の比較`}>
        ...
        <polyline className="chart-line chart-reference" points={chartPoints(reference, min, max)} /><polyline className="chart-line chart-current" points={chartPoints(current, min, max)} />
      </svg>
...
function DimensionCard({ dimension }: { dimension: ToneDimension }) {
  ...
  return (
    <article className="dimension-card"><div className="dimension-heading"><div><h3>{dimension.label}</h3><p>{directionLabel(dimension.difference)}</p></div><strong>{dimension.difference > 0 ? "+" : ""}{dimension.difference.toFixed(0)}</strong></div>
      ...
      <p className="interpretation">{dimension.interpretation}</p><ul>{dimension.evidence.map((item) => <li key={item}>{item}</li>)}</ul><p className="suggestion"><span>調整方向</span>{dimension.suggestion}</p>
    </article>
  );
}
...
export default function Home() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!reference || !current) { setError("参考音と自分の音を両方選択してください。"); return; }
    setLoading(true); setError(null); setResult(null);
    try { setResult(await compareAudio(reference, current)); } catch (caught) { setError(caught instanceof Error ? caught.message : "解析に失敗しました。"); } finally { setLoading(false); }
  }
  return (
    <main>
      <header className="hero"><p className="brand">TONE LAB / PROTOTYPE 0.1</p><h1>音の違いを、<br />調整できる言葉にする。</h1><p className="lead">参考音と自分のギター音を比較し、明るさ・太さ・アタック・圧縮感・粗さの差を表示します。</p></header>
      <form className="workspace" onSubmit={handleSubmit}><div className="upload-grid"><AudioInput id="reference" label="REFERENCE / 参考音" file={reference} onChange={setReference} /><AudioInput id="current" label="CURRENT / 自分の音" file={current} onChange={setCurrent} /></div><div className="conditions"><span>同じフレーズ推奨</span><span>開始位置を自動補正</span><span>最大30秒</span><span>WAV / MP3 / FLAC / OGG</span></div><button className="analyze-button" type="submit" disabled={loading}>{loading ? "解析中…" : "2つの音を比較する"}</button>{error ? <p className="error" role="alert">{error}</p> : null}</form>
      {result && reference && current ? <section className="results"><div className="result-title"><p className="eyebrow">COMPARISON</p><h2>主な違い</h2></div><AlignmentStatus alignment={result.alignment} /><QualityStatus quality={result.quality} /><ABAudition referenceFile={reference} currentFile={current} result={result} /><div className="summary-grid">{result.summary.map((item) => <p key={item}>{item}</p>)}</div><AdjustmentPlanSection steps={result.adjustment_plan} /><div className="legend"><span>− 参考音が強い</span><span>0 近い</span><span>＋ 自分の音が強い</span></div><div className="dimension-grid">{result.dimensions.map((dimension) => <DimensionCard key={dimension.key} dimension={dimension} />)}</div><ComparisonVisualsSection visuals={result.visuals} /><details className="raw-data"><summary>測定値を見る</summary><pre>{JSON.stringify({ alignment: result.alignment, quality: result.quality, reference: result.reference, current: result.current }, null, 2)}</pre></details><p className="disclaimer">{result.disclaimer}</p></section> : null}
    </main>
  );
}
